...
static std::wstring GetDeviceName() {
	wchar_t name[] = { L'\\', L'\\', L'.', L'\\', L'N', L'a', L'l', L'\0' };
	return std::wstring(name);
}

bool intel_driver::IsRunning() {
	const HANDLE file_handle = CreateFileW(GetDeviceName().c_str(), FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
NTSTATUS intel_driver::Load() {
	...
	hDevice = CreateFileW(GetDeviceName().c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
uint64_t intel_driver::AllocatePool(nt::POOL_TYPE pool_type, uint64_t size) {
	...
	static const ULONG pool_tags[] = {
	'TcpE','UdpE','Tcp6','Udp6',
	'LDmp','NDmp','FMfn','FMsl'
	};
	ULONG tag = pool_tags[rand() % (sizeof(pool_tags) / sizeof(pool_tags[0]))];
	if (!CallKernelFunction(&allocated_pool, kernel_ExAllocatePool, pool_type, size, tag)) //Changed pool tag since an extremely meme checking diff between allocation size and average for detection....
		return 0;
