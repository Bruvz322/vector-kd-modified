...
NTSTATUS service::RegisterAndStart(const std::wstring& driver_path, const std::wstring& serviceName) {
	...
	// fake description to look like legitimate service
	static const wchar_t* fake_descriptions[] = {
		L"Windows Audio Device Graph Isolation",
		L"Manages audio for Windows-based programs",
		L"Network Connection Manager",
		L"Provides network connectivity status notifications"
	};
	const wchar_t* desc = fake_descriptions[rand() % 4];
	RegSetKeyValueW(dservice, NULL, L"Description", REG_SZ, desc, (DWORD)(wcslen(desc) * sizeof(wchar_t)));
